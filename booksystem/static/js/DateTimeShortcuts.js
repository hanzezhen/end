var DateTimeShortcuts = {
    ...
    openCalendar: function(num) {
        ...
        var inp = document.getElementById("searchbar"+num)
...
//提交前验证,输入要从1991到9999，考虑了平年和润年
function check1(){

    starttime=document.getElementById("searchbar0").value

    if(typeof starttime == "undefined" || starttime == null || starttime == ""){
        starttime="2019/01/01"
    }

    endtime=document.getElementById("searchbar1").value
    ...
    var errorinput=document.getElementById("errorinfo")

    var result=starttime.match(/^(\d{4})(\/)(\d{1,2})(\/)(\d{1,2})$/);
    var result1=endtime.match(/^(\d{4})(\/)(\d{1,2})(\/)(\d{1,2})$/);
    if(result==null || result1==null) {

        errorinput.value="请按正确的格式输入日期"
        return false;

    }
    // alert("result:"+result)
    //alert(result[1]+"#"+result[3]-1+"#"+result[4])
    if(result[1]!="0000" && result[3]!="00" && result[5]!="00" && result1[1]!="0000" && result1[3]!="00" && result1[5]!="00"){
        var d=new Date(result[1], result[3]-1, result[5]);
        var d1=new Date(result1[1], result1[3]-1, result1[5]);
        // alert(d)
        if(d.getFullYear()==result[1] && d.getMonth()+1==result[3] && d.getDate()==result[5] && d1.getFullYear()==result1[1] && d1.getMonth()+1==result1[3] && d1.getDate()==result1[5]){
